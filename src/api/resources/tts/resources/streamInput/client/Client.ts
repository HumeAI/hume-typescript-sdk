...
export class StreamInputClient {
  ...
  public async connect(
    args: StreamInputClient.ConnectArgs = {},
  ): Promise<StreamInputSocket> {
    const {
      accessToken,
      contextGenerationId,
      formatType,
    ...
    if (formatType != null) {
      _queryParams.format_type = serializers.tts.AudioFormatType.jsonOrThrow(
        formatType,
        {
    ...
    if (includeTimestampTypes != null) {
      if (Array.isArray(includeTimestampTypes)) {
        _queryParams.include_timestamp_types = includeTimestampTypes.map(
          (item) =>
        ...
        _queryParams.include_timestamp_types =
    ...
    if (version != null) {
      _queryParams.version = serializers.tts.OctaveVersion.jsonOrThrow(
        version,
        {
          unrecognizedObjectKeys: 'strip',
          omitUndefined: true,
