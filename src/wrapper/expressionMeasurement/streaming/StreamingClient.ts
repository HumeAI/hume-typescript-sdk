...
export declare namespace StreamClient {
  ...
  interface ConnectArgs {
    ...
    onMessage?: (
      message: Hume.expressionMeasurement.stream.StreamBurst,
    ) => void;
...
export class StreamClient {
  ...
  public connect(args: StreamClient.ConnectArgs): StreamSocket {
    const websocket = new WebSocket(`wss://api.hume.ai/v0/stream/models`, {
      headers: {
        'X-Hume-Api-Key':
          typeof this._options.apiKey === 'string' ? this._options.apiKey : '',
      },
      timeout: 10,
    });
...
export async function parse(
  ...
  | Hume.expressionMeasurement.stream.StreamBurst
  | Hume.expressionMeasurement.stream.StreamError
  | undefined
> {
  const message = JSON.parse(data as string);

  const parsedResponse =
    await serializers.expressionMeasurement.stream.StreamBurst.parse(message, {
  ...
  const parsedError =
    await serializers.expressionMeasurement.stream.StreamError.parse(message, {
      unrecognizedObjectKeys: 'passthrough',
      allowUnrecognizedUnionMembers: true,
      allowUnrecognizedEnumValues: true,
      breadcrumbsPrefix: ['response'],
