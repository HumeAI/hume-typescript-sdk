...
test("UnclockedSilenceFiller", () => {
    ...
    const validateBufferSegments = (
        buffer: Buffer | null,
        expectedSegments: { type: "sound" | "silence"; bytes: number }[],
    ): void => {
        if (!buffer) {
    ...
    test("scenario", () => {
        const filler = new UnclockedSilenceFiller(prebufferBytes, sampleRate, bytesPerSample);
        ...
        const timeline = [
            ...
            [
                ...
                "read",
                [
                    { type: "sound", bytes: msOfAudioBytes(100) },
                    { type: "silence", bytes: msOfAudioBytes(100) },
                ],
            ],
